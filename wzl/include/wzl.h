...
namespace wzl {
	...
	//wzl Map容器
	typedef std::unordered_map<int, WzlBmpTex*> UWzlTexMap;

	class WZL_EXPORT Wzl {
	private:
		const char* path;			//文件路径
		void* wzxData;				//wzx数据
		int wzxSize;				//wzx总大小
		WzxHeader wzxHead;			//wzx头部信息

		void* wzlData;				//wzl数据
		int wzlSize;				//wzl总大小
		WzlHeader wzlHead;			//wzl头部信息

		UWzlTexMap* _wzlTexMap;		//wzl缓存纹理map
		uge::UGE* pUge;
	public:
	...
	private:
		bool _LoadWzl();
		bool _LoadWzx();
		byte* _GetBmpData(int sort, WzlBmpInfo* wzlBmp, int* dstSize);
